...
</style></head><body><div class="card"><h1>狗骨本地酒馆 · Node 已保护启动</h1><p class="muted">APP 没有闪退，但本地 SillyTavern 启动失败。通常是内置 Node 版本与当前 SillyTavern 版本不匹配，或者移动端缺少依赖。</p><p>错误信息：</p><pre>${htmlEscape(message)}</pre><p class="muted">把这个页面截图给我，我就能继续精确修。</p></div></body></html>`);
  });
...
// 很重要：SillyTavern 或依赖如果调用 process.exit，嵌入式 Node 可能直接把整个 APP 进程杀掉。
// 这里拦截 process.exit，避免出现“界面显示 Node 启动中然后 APP 闪退”。
...
try {
  ...
  log('cwd=' + process.cwd());
  log('starting SillyTavern on 127.0.0.1:' + port);

  try {
    require(serverJs);
  } catch (e) {
    log('SillyTavern require(server.js) failed: ' + (e.stack || e.message || e));
